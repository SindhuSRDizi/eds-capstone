...
function toggleMenu(nav, navSections, forceExpanded = null) {
  // Check if nav and navSections exist before proceeding
  if (!nav) {
    console.error('The "nav" element is missing.');
    return; // Exit the function if nav is null
  }

  if (!navSections) {
    console.error('The "navSections" element is missing.');
    return; // Exit the function if navSections is null
  }
  ...
  if (!button) {
    console.error('The button inside nav-hamburger is missing.');
    return; // Exit the function if button is not found
  }
...
export default async function decorate(block) {
  ...
  if (!nav.firstElementChild) {
    console.error('Failed to load nav fragment. Ensure the path and content are correct.');
    return; // Exit early if nav loading fails
  }
  ...
  const brandImage = createElement('img', { attributes: { src: `${window.hlx.codeBasePath}/img/wknd-logo.svg`, alt: "Brand Logo" } });
  ...
  if (nav && navSections) {
    ...
    console.warn('Nav or Nav Sections are not available at the time of initialization.');
  }
